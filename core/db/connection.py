...
def uvicorn_worker_count() -> int:
    raw = os.getenv("WEB_CONCURRENCY") or os.getenv("UVICORN_WORKERS") or "4"
    try:
...
def warn_if_pool_near_pg_limit(
    ...
    if potential < int(pg_max_connections * 0.8):
        return None
    return (
        f"Postgres pool max_size={max_size} × workers={workers} = {potential} "
...
async def init_db():
    ...
    pool_warn = warn_if_pool_near_pg_limit(PG_POOL_MAX_SIZE, uvicorn_worker_count())
    if pool_warn:
