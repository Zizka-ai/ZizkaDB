  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          style={{ display: 'flex', alignItems: 'center', gap: 16, fontSize: 13, color: '#737373' }}
        >
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {summary.daily_new_installs.length > 0 && (
        <Card title='New installs (last 30 days)' subtitle='Daily count of first-ever pings.'>
          <DailyBars
            data={summary.daily_new_installs.map((d) => ({ day: d.day, value: d.new_installs }))}
            accent='#f97316'
          />
        </Card>
      )}
            ...
            <tbody>
              {recent.map((r) => (
                <tr key={r.install_id + r.last_seen} style={{ borderBottom: '1px solid #161616' }}>
                  ...
                  <Td>{r.os}</Td>
                  <Td>
                    <Tag color={r.mode === 'cloud' ? '#22c55e' : '#f97316'}>{r.mode}</Tag>
                  </Td>
                  <Td align='right' mono>
                    {r.ping_count}
                  </Td>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      {overview && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          ...
          <Stat
            label='Stripe linked'
            value={fmt(overview.stripe_linked)}
            sub='has customer id in Stripe'
            accent='#f97316'
          />
        </div>
      )}
      ...
      <Card
        ...
        subtitle='Everyone on a Pro/Team trial or paid plan. Email = signup identity. Stripe column links to Stripe Dashboard when checkout has run.'
      >
        {!rows ? (
          <SkeletonBlock />
        ) : rows.length === 0 ? (
          <Empty>
            No subscribers yet. Run DB migration 002_user_billing.sql on the server, then restart
            API. New signups get a 30-day Pro trial automatically.
          </Empty>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table
              style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 880 }}
            >
              <thead>
                <tr
                  style={{
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {usage && usage.daily.length > 0 && (
        ...
        >
          ...
          <div style={{ marginTop: 16, fontSize: 12, color: '#737373' }}>
            Peak day:{' '}
            {usage.daily.reduce((a, b) => (b.events > a.events ? b : a), usage.daily[0]).day} ·{' '}
      ...
      >
        {!users ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table
              style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5, minWidth: 900 }}
            >
              <thead>
                <tr
                  style={{
              ...
              <tbody>
                {users.map((u) => (
                  <tr
                    key={u.user_id}
                    style={{ borderBottom: '1px solid #161616', verticalAlign: 'top' }}
                  >
                    <Td>
                      <div style={{ fontWeight: 500 }}>{u.email}</div>
                      <div
                    ...
                    <Td align='right' mono>
                      <span style={{ color: u.events_7d > 0 ? '#22c55e' : '#525252' }}>
                        {fmt(u.events_7d)}
                      </span>
                    </Td>
                    <Td align='right' mono>
...
function DemoRequestsSection({ token }: { token: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Card
        title='Demo requests'
        subtitle='Submissions from the homepage Book demo form. Newest first.'
      >
        {!rows ? (
          <SkeletonBlock />
        ) : rows.length === 0 ? (
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 16 }}>
        ...
        {subtitle && (
          <div style={{ fontSize: 12, color: '#737373', lineHeight: 1.5 }}>{subtitle}</div>
        )}
      </div>
      {children}
    </div>
            ...
            return (
              <div key={r.k}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                  <span style={{ fontSize: 12, color: '#d4d4d4', fontFamily: 'monospace' }}>
                    {r.k || '—'}
                  </span>
                  <span style={{ fontSize: 12, color: '#737373', fontFamily: 'monospace' }}>
                    {fmt(r.n)}
                  </span>
                </div>
                <div
                  style={{ height: 4, background: '#0a0a0a', borderRadius: 2, overflow: 'hidden' }}
                >
                  <div
                    style={{ height: '100%', width: `${pct}%`, background: accent, opacity: 0.7 }}
                  />
                </div>
              </div>
            );
...
function Th({ children, align = 'left' }: { children: React.ReactNode; align?: 'left' | 'right' }) {
  return (
    <th style={{ padding: '10px 12px', textAlign: align, fontWeight: 600, letterSpacing: 0.8 }}>
      {children}
    </th>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          style={{ height: 18, background: '#161616', borderRadius: 4, opacity: 0.5 - i * 0.15 }}
        />
      ))}
    </div>
  );
...
function Empty({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ padding: '32px', textAlign: 'center', color: '#525252', fontSize: 13 }}>
      {children}
    </div>
  );
}
