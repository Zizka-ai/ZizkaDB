...
export default function CommunityPage() {
  ...
  const load = useCallback(async () => {
    ...
    setErr('')
    try {
      const data = await listCommunityPosts(filter === 'all' ? undefined : filter)
      setPosts(data)
    } catch (e) {
    ...
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', color: '#111', background: '#fafafa', minHeight: '100vh' }}>
      ...
      <main className="zdb-comm-pad" style={{ maxWidth: 760, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <p style={{ fontSize: 15, color: '#555', lineHeight: 1.6, margin: '0 0 24px' }}>
          Share experiences, post screenshots, ask questions, and help other builders. No account required — just your name.
        </p>
        ...
        <button
          ...
          style={{
            ...
            background: showForm ? '#fff' : '#111', color: showForm ? '#111' : '#fff',
            border: showForm ? '1px solid #ddd' : 'none', borderRadius: 10,
        ...
        {err && (
          <p style={{ color: '#ef4444', fontSize: 13, marginBottom: 16 }}>{err}</p>
        )}

        {loading ? (
          <p style={{ color: '#888', fontSize: 14 }}>Loading…</p>
        ) : posts.length === 0 ? (
          <div style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: 12, padding: 32, textAlign: 'center' }}>
            <p style={{ color: '#888', margin: 0 }}>No posts yet. Be the first to share something.</p>
          </div>
        ) : (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{
                    ...
                    color: CAT_COLOR[p.category as CommunityCategory] ?? '#888',
                  }}>
                    {p.category}
                  </span>
                ...
                <h2 style={{ fontSize: 16, fontWeight: 600, margin: '0 0 6px' }}>{p.title}</h2>
                <p style={{ fontSize: 14, color: '#555', margin: 0, lineHeight: 1.5 }}>{p.excerpt}{p.excerpt.length >= 280 ? '…' : ''}</p>
...
function NewPostForm({
  ...
  const inputStyle: React.CSSProperties = {
    ...
    border: '1px solid #ddd', borderRadius: 8, fontSize: 14, marginBottom: 12,
  }

  return (
    <form onSubmit={submit} style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: 12, padding: 20, marginBottom: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          ...
          <input required value={authorName} onChange={(e) => setAuthorName(e.target.value)} style={inputStyle} placeholder="Alex" />
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: '#555' }}>Email (optional)</label>
          <input type="email" value={authorEmail} onChange={(e) => setAuthorEmail(e.target.value)} style={inputStyle} placeholder="you@company.com" />
        </div>
      </div>

      <label style={{ fontSize: 12, fontWeight: 600, color: '#555' }}>Type</label>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {(['question', 'experience', 'showcase'] as CommunityCategory[]).map((c) => (
          <button
            key={c}
            type="button"
      ...
      <input required value={title} onChange={(e) => setTitle(e.target.value)} style={inputStyle} placeholder="What's on your mind?" />

      <label style={{ fontSize: 12, fontWeight: 600, color: '#555' }}>Details *</label>
      <textarea
        required
        value={body}
      ...
      <label style={{ fontSize: 12, fontWeight: 600, color: '#555' }}>Screenshots (optional)</label>
      <input type="file" accept="image/png,image/jpeg,image/webp,image/gif" multiple onChange={onFile} style={{ fontSize: 13, marginBottom: 12 }} />
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
          {images.map((url) => (
            <div key={url} style={{ position: 'relative' }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={mediaUrl(url)} alt="" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: 8 }} />
              <button
                type="button"
                onClick={() => setImages((prev) => prev.filter((u) => u !== url))}
                style={{
                  position: 'absolute', top: 4, right: 4, width: 20, height: 20,
                  borderRadius: '50%', border: 'none', background: '#111', color: '#fff',
      ...
      <button
        type="submit"
        disabled={submitting || !authorName.trim() || title.length < 3 || body.length < 10}
        style={{
          padding: '11px 20px', background: '#111', color: '#fff', border: 'none',
