    ...
    <Shell agentId={agentId} lastSync={lastSync} onDelete={handleDeleteAgent} deleting={deleting}>
      ...
      >
        {(
          ...
          ] as { key: Tab; label: string; icon: React.ElementType; badge?: 'NEW' }[]
            ...
            {sessLoading ? (
              ...
              <div className='text-sm py-8 text-center' style={{ color: '#e5e5e5' }}>
                ...
                <span className='text-xs'>
                  Pass a <code className='text-green-400'>session_id</code> when logging events.
                </span>
              </div>
            ) : (
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      <span className='text-xs' style={{ color: '#e5e5e5' }}>
                        {s.duration_seconds > 0
                          ? formatDuration(
                              intervalToDuration({ start: 0, end: s.duration_seconds * 1000 }),
                    ...
                    >
                      {(sessDiff as { new_event_types?: string[] }).new_event_types?.length ? (
                        <span style={{ color: '#22c55e' }}>
                          {(sessDiff as { new_event_types: string[] }).new_event_types.length} new
                          event type
                          {(sessDiff as { new_event_types: string[] }).new_event_types.length !== 1
                            ? 's'
                            : ''}
                        </span>
                      ) : (
      ...
      {tab === 'timetravel' && (
        <div className='max-w-2xl'>
          ...
          >
            ...
            Reconstruct logged state for <span className='font-mono text-white'>{agentId}</span> at
            any point in time. See every event that had happened up to that moment.
          </div>
      ...
      {[
        ...
        { label: 'Sessions', value: stats.sessions, icon: Layers, color: '#e5e5e5' },
        {
          ...
          value: stats.last_event
            ? formatDistanceToNow(new Date(stats.last_event), { addSuffix: true })
            : '—',
          icon: Clock,
          color: '#e5e5e5',
        },
        {
          ...
          value: errorCount,
          icon: AlertCircle,
          color: errorCount > 0 ? '#ef4444' : '#e5e5e5',
        },
                ...
                <div key={ev.event_id}>
                  ...
                  {isSelected && (
                    ...
                    >
                      ...
                      >
                        <span>
                          id: <span className='font-mono'>{ev.event_id.slice(0, 18)}…</span>
                        </span>
                        {ev.session_id && (
                          <span>
                            session:{' '}
                            <span className='font-mono'>{ev.session_id.slice(0, 12)}…</span>
                          </span>
                        )}
  ...
  return (
    <div>
      <div className='flex items-center gap-2 mb-3'>
        ...
        <span className='font-mono font-semibold text-sm' style={{ color: '#e5e5e5' }}>
          {event.event}
        </span>
      </div>
      <pre
      ...
      <div className='space-y-4'>
        ...
        >
          ...
          <p className='text-sm' style={{ color: '#fdba74' }}>
            ...
            <span className='font-mono'>{agentId}</span> has more than {recent_window} sessions.
          </p>
        </div>
        {recent && (
          <WindowCard
      ...
      <div className='flex items-start justify-between gap-4 mb-3'>
        <div>
          ...
          <h2 className='text-lg font-semibold' style={{ color: '#fff' }}>
            ...
            <span className='text-sm font-normal ml-2' style={{ color: '#e5e5e5' }}>
              ({scorePct} / 100)
            </span>
          </h2>
    ...
    <div className='px-4 py-2.5 flex items-center gap-3'>
      ...
      <div className='flex-1 min-w-0'>
        <div className='flex items-baseline gap-2'>
          <span className='text-xs uppercase tracking-wider' style={{ color: '#e5e5e5' }}>
            {scopeLabel}
          </span>
          <span className='text-sm font-mono truncate' style={{ color: '#e5e5e5' }}>
            {detail}
          </span>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className='p-4 space-y-4'>
        ...
        {/* Event distribution */}
        <div>
          <div className='text-xs font-medium mb-2' style={{ color: '#e5e5e5' }}>
            Event distribution
          </div>
          {topEvents.length === 0 ? (
  ...
  return (
    <div>
      <div className='flex items-center justify-between mb-0.5'>
        <span className='text-xs font-mono truncate pr-2' style={{ color: '#d4d4d4' }}>
          {label}
        </span>
        <span className='text-xs font-mono shrink-0' style={{ color: '#e5e5e5' }}>
          {pct.toFixed(1)}%
        </span>
      </div>
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div className='flex items-start gap-2 text-xs'>
      <span className='font-mono shrink-0' style={{ color: '#e5e5e5', minWidth: 80 }}>
        {label}
      </span>
