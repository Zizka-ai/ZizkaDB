...
export default function SettingsPage() {
  ...
  const [catalog, setCatalog] = useState<
    { id: string; models: { id: string; label: string; description?: string }[] }[]
  >([]);
  ...
  useEffect(() => {
    ...
    getEmbeddingSettings(token)
      .then(
        (s: { provider?: string; model?: string; use_platform_key?: boolean; ready?: boolean }) => {
  ...
  return (
    <div className='p-8 max-w-2xl mx-auto'>
      ...
      <p
        ...
        style={{ color: '#e5e5e5', background: '#1a1a1a', border: '1px solid #2a2a2a' }}
      >
        API keys belong to an agent.{' '}
        <Link href='/dashboard' className='underline' style={{ color: '#22c55e' }}>
          Create an agent
        </Link>{' '}
        to get a key, or open an agent to add more keys. Legacy tenant-wide keys (no agent) still
        work.
      </p>
      ...
      >
        ...
        <p className='text-xs mb-4' style={{ color: '#e5e5e5' }}>
          Choose the model used for semantic search and context injection (like Pinecone&apos;s
          embedding choice). All models use 1536 dimensions. New events use this model; existing
          vectors are not re-indexed automatically.
        </p>
        {embLoading ? (
      ...
      >
        ...
        <p className='text-xs mb-4' style={{ color: '#e5e5e5' }}>
          Logs to agent <span className='font-mono'>dashboard-connection-test</span> (not your app
          agent). To test a specific agent, open that agent and click <strong>Test agent</strong>.
        </p>
        <button
          type='button'
          disabled={testBusy}
      ...
      >
        ...
        <p className='text-xs mb-3' style={{ color: '#e5e5e5' }}>
          ...
          <span className='font-mono'>conv-user1</span>,{' '}
          <span className='font-mono'>conv-user2</span>). Most users should create per-agent keys on
          the{' '}
          <Link href='/dashboard' className='underline' style={{ color: '#22c55e' }}>
            Agents
          </Link>{' '}
          ...
          >
            ...
            <div className='flex items-center gap-2'>
              ...
              <button
                type='button'
                onClick={() => {
                  navigator.clipboard.writeText(tenantNewKey);
                  setCopied(true);
