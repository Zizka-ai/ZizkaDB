...
function SignupPlanInner() {
  ...
  useEffect(() => {
    ...
    const stored = typeof window !== 'undefined' ? sessionStorage.getItem('signup_plan') : null;
    const preselect =
    ...
    async function init() {
      try {
        const [config, status] = await Promise.all([getBillingConfig(), getBillingStatus(token!)]);
        if (cancelled) return;
    ...
    >
      <div style={{ width: '100%', maxWidth: 720 }}>
        ...
        >
          ...
          >
            {trialDays}-day free trial on every plan. Card required — you won&apos;t be charged
              ...
              {plans.map((plan) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 14 }}
                    >
                    ...
                    >
                      {plan.features.map((f) => (
                        <li
                          key={f}
                          style={{ fontSize: 13, color: '#333', display: 'flex', gap: 8 }}
                        >
                    ...
                    {isSelected && (
                      <p
                        style={{ margin: '14px 0 0', fontSize: 12, fontWeight: 700, color: BRAND }}
                      >
                        Selected
                      </p>
                    )}
