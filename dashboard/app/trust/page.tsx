...
export default function TrustPage() {
      ...
      <div style={{ display: 'flex', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <main style={{ flex: 1, maxWidth: 780, padding: '48px 28px 96px', minWidth: 0 }}>
          ...
          <p style={{ fontSize: 17, color: '#444', lineHeight: 1.75, marginBottom: 48 }}>
            The operational database for AI agents. This document describes architecture, APIs, data
            semantics, and how ZizkaDB fits alongside vector databases, application databases, and
            tracing tools.
          </p>

          <Section id='overview' title='Overview'>
            <p style={p}>
              ZizkaDB stores <strong>agent events</strong> — decisions, tool calls, messages, and
              outcomes — with causal links, semantic search over history, reconstruction of logged
              state at a timestamp, and statistical behavioral baselines per agent.
            </p>
            <table style={table}>
              <thead>
                <tr>
 ...
 └── OTel / LangSmith   →  distributed traces (optional)`}</pre>
          ...
          <Section id='problem' title='Problem domain'>
            ...
            <ul style={ul}>
              <li>
                <strong>Causal debugging</strong> — walk from a bad output to the event that
                triggered it
              </li>
              <li>
                <strong>State at time T</strong> — what was logged for an agent before a given
                timestamp
              </li>
              <li>
                <strong>Cross-session memory</strong> — under your API keys, retention policy, and
                erasure controls
              </li>
              <li>
                <strong>Behavior change detection</strong> — compare recent sessions to that
                agent&apos;s historical pattern
              </li>
            </ul>
            <p style={p}>
              ZizkaDB targets these operational concerns. It does not replace embedding indexes for
              document RAG or distributed tracing for infrastructure spans.
            </p>
          </Section>
          ...
          <Section id='architecture' title='Architecture'>
            ...
            <p style={p}>
              Docker Compose stack: Postgres (pgvector), Qdrant, Redis, API. Dashboard is optional
              (run from <code style={codeInline}>dashboard/</code>). Requires{' '}
              <code style={codeInline}>OPENAI_API_KEY</code> for embedding-backed features; logging
              and causal APIs work without it.
            </p>
...
docker compose -f infra/docker-compose.yml up -d`}</Code>
          ...
          <Section id='data-model' title='Data model'>
            ...
            <p style={p}>
              ...
              <code style={codeInline}>zizkadb_live_*</code>) scope all reads and writes to a
              tenant. Dashboard auth uses email OTP → JWT.
            </p>
          </Section>
          ...
          <Section id='capabilities' title='Capabilities'>
            {[
              ...
              <div
                key={cap.name}
                style={{ marginBottom: 20, paddingBottom: 20, borderBottom: '1px solid #f0f0f0' }}
              >
          ...
          <Section id='time-travel' title='Time travel semantics'>
            <p style={p}>
              ...
              <strong>logged state</strong> by aggregating all events for that agent with{' '}
              <code style={codeInline}>timestamp ≤ T</code>. The returned structure reflects what
              was recorded in ZizkaDB, not a re-execution of the LLM or tools.
            </p>
            <table style={table}>
              <thead>
                <tr>
          ...
          <Section id='integrity' title='Integrity & retention'>
            <ul style={ul}>
              <li>
                Events are stored in <strong>Postgres</strong> with an append-by-default write
                pattern; each payload includes a <strong>SHA-256 checksum</strong> for integrity
                verification.
              </li>
              <li>
                <code style={codeInline}>forget()</code> removes events matching metadata filters
                (GDPR right to erasure) — storage is not WORM/immutable.
              </li>
              <li>
                Managed plans enforce <strong>retention windows</strong> (90 days Pro, 1 year Team);
                self-hosted retention is operator-defined.
              </li>
              <li>
                Bulk signed audit export is on the product roadmap; today, export via API/query and
                verify checksums per event.
              </li>
            </ul>
          </Section>
          ...
          <Section id='performance' title='Performance expectations'>
            <p style={p}>
              ZizkaDB is built for normal agent loops — tool calls, messages, and decisions — not
              for blockchain-scale write throughput. Stack: Postgres (events), Qdrant (vectors),
              Redis (cache).
            </p>
            <table style={table}>
              <thead>
          ...
          <Section id='security' title='Security (early stage)'>
            <p style={p}>
              ZizkaDB v1 is aimed at developers and small teams — not enterprise compliance
              certification yet. Here is what we do today:
            </p>
            <ul style={ul}>
              <li>
              ...
              <li>
                <strong>Tenant isolation:</strong> every API call scoped by API key or JWT to a{' '}
                <code style={codeInline}>tenant_id</code>; no cross-tenant reads.
              </li>
              <li>
                <strong>At rest:</strong> standard Postgres / disk encryption on the operator&apos;s
                infrastructure (AWS EBS on managed; your disk when self-hosted).
              </li>
              <li>
                <strong>BYOK embedding keys:</strong> encrypted in Postgres (Fernet) when you bring
                your own OpenAI key.
              </li>
              <li>
                <strong>GDPR erasure:</strong> <code style={codeInline}>forget()</code> deletes
                matching events and vectors.
              </li>
              <li>
                <strong>Telemetry:</strong> one anonymous SDK/MCP startup ping — opt out with{' '}
                <code style={codeInline}>ZIZKADB_TELEMETRY=false</code>.
              </li>
            </ul>
            <p style={p}>
              <strong>Not claimed today:</strong> SOC 2, HIPAA, or formal DPAs. For enterprise
              security review, contact{' '}
              <a href='mailto:founder@zizka.ai' style={link}>
                founder@zizka.ai
              </a>
              .
            </p>
          ...
          <Section id='integration' title='Integration'>
            <p style={p}>
              ...
              <strong>stateless HTTP clients</strong> — no thread-local storage or{' '}
              <code style={codeInline}>contextvars</code>. Pass{' '}
              <code style={codeInline}>agent</code>, <code style={codeInline}>session_id</code>,{' '}
              <code style={codeInline}>parent_id</code>, and{' '}
              <code style={codeInline}>event_id</code> explicitly on each call. Safe in FastAPI,
              Celery, and parallel async workers.
            </p>
            <table style={table}>
              <thead>
                <tr>
            ...
            <p style={p}>
              Cloud default host: <code style={codeInline}>https://db.zizka.ai</code>. Self-host:
...
chain = await db.why(tool.event_id)`}</Code>
            ...
            <p style={p}>
              Add to <code style={codeInline}>~/.cursor/mcp.json</code> or project{' '}
              <code style={codeInline}>.cursor/mcp.json</code>, then reload MCP:
            </p>
            <Code lang='json'>{`{
  "mcpServers": {
    "zizkadb": {
    ...
    "zizkadb": {
      ...
      "env": { "ZIZKADB_HOST": "http://localhost:8000" }
            ...
            <p style={p}>
              ...
              <code style={codeInline}>get_context</code>, <code style={codeInline}>why</code>,{' '}
              <code style={codeInline}>query_events</code>,{' '}
              <code style={codeInline}>time_travel</code>,{' '}
              <code style={codeInline}>memory_diff</code>, <code style={codeInline}>forget</code> —
              see{' '}
            ...
            <p style={p}>
              Telemetry: one anonymous SDK startup ping (name, version, OS, cloud vs self-host). Opt
              out: <code style={codeInline}>ZIZKADB_TELEMETRY=false</code>.
            </p>
          </Section>

          <Section id='api' title='REST API'>
            <p style={p}>
              Base URL <code style={codeInline}>https://db.zizka.ai</code>. Auth:{' '}
              <code style={codeInline}>Authorization: Bearer &lt;token&gt;</code>. Interactive
              schema:{' '}
          ...
          <Section id='deployment' title='Deployment'>
            ...
            <p style={p}>
              ...
              → email OTP → Settings → API key → SDK or MCP env.
            </p>
          </Section>

          <Section id='comparison' title='Comparison'>
            <p style={p}>
              Capability matrix (May 2026). Verify competitor docs before external debates; ~ =
              partial support.
            </p>
          ...
          <Section id='licensing' title='Licensing'>
            ...
            <p style={p}>
              AGPL applies when you modify and distribute the server. MCP is MIT for IDE
              integration. Commercial embedding requires legal review.
            </p>
          </Section>
          ...
          <Section id='limits' title='Plan limits'>
            ...
            <p style={p}>
              Baseline/drift requires sufficient <code style={codeInline}>session_id</code>{' '}
              coverage; early agents report <code style={codeInline}>warming_up</code> or{' '}
