...
export function CalendlyBookModal({ open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    loadCalendlyScript()
      ...
      .catch(() => {
        if (!cancelled)
          setLoadErr('Could not load the scheduler. Try again or open Calendly in a new tab.');
      });
      ...
      >
        {booked ? (
          <div style={{ padding: '32px 28px', textAlign: 'center' }}>
            <h2
              id='book-demo-title'
              style={{ margin: '0 0 12px', fontSize: 22, fontWeight: 800, color: '#000' }}
            >
              You&apos;re booked
            </h2>
            <p
            ...
            <button
              type='button'
              onClick={onClose}
              style={{ ...violetBtn, border: 'none', cursor: 'pointer' }}
            >
              Close
            </button>
          </div>
          ...
          <>
            ...
            >
              <h2
                id='book-demo-title'
                style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#000' }}
              >
            ...
            {loadErr ? (
              <div style={{ padding: '16px 20px 24px' }}>
                <p style={{ margin: '0 0 16px', fontSize: 14, color: '#b91c1c', fontWeight: 600 }}>
                  {loadErr}
                </p>
                <a
                  href={CALENDLY_URL}
                  target='_blank'
                  rel='noreferrer'
                  style={{ ...violetBtn, display: 'inline-flex' }}
                >
                  Open Calendly →
                </a>
              </div>
