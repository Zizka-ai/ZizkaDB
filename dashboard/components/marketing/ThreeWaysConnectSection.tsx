  ...
  return (
    <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11.5, color: '#000', lineHeight: 1.5, fontWeight: 500 }}>
          {children}
        </div>
      </div>
    </div>
...
export function ThreeWaysConnectSection() {
  return (
    <section
      ...
      style={{ padding: '56px 40px 64px', background: '#fff' }}
    >
        ...
        >
          ...
          >
            <ConnectStep n={1} title='Create account' accent={M.violet}>
              <Link href='/signup' style={{ color: M.violet, fontWeight: 700 }}>
                Sign up
              </Link>{' '}
              — email OTP, no card.
            </ConnectStep>
            <ConnectStep n={2} title='Create agent & key' accent={M.violet}>
              <Link href='/dashboard' style={{ color: M.violet, fontWeight: 700 }}>
                Dashboard
              </Link>{' '}
              → create agent → copy{' '}
              <code style={{ fontFamily: 'monospace', fontSize: 10 }}>zizkadb_live_…</code>
            </ConnectStep>
...
await db.log(agent="my-bot", event="started", data={})`}</CodeBlock>
          ...
          >
            <ConnectStep n={1} title='Install' accent={M.blue}>
              <code style={{ fontFamily: 'monospace', fontSize: 10 }}>pip install zizkadb-mcp</code>{' '}
              or <code style={{ fontFamily: 'monospace', fontSize: 10 }}>uvx zizkadb-mcp</code>
            </ConnectStep>
            <ConnectStep n={2} title='Paste & reload' accent={M.blue}>
              Add to Cursor <code style={{ fontFamily: 'monospace', fontSize: 10 }}>mcp.json</code>{' '}
          ...
          >
            <ConnectStep n={1} title='Clone & start' accent={M.teal}>
              <a
                ...
                target='_blank'
                rel='noreferrer'
                style={{ color: M.teal, fontWeight: 700 }}
              >
                GitHub
              </a>{' '}
              →{' '}
...
await db.log(agent="my-bot", event="started", data={})`}</CodeBlock>
            <p style={{ fontSize: 11, margin: '10px 0 0', fontWeight: 700 }}>
              <a href={GITHUB_URL} target='_blank' rel='noreferrer' style={{ color: M.teal }}>
                GitHub →
              </a>
              {' · '}
              <Link href='/docs' style={{ color: '#000' }}>
                Docs
              </Link>
            </p>
          </ConnectCard>
