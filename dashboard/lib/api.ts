...
export async function getEvents(token: string, agent: string, params: Record<string, string> = {}) {
...
export async function getAgentBaseline(token: string, agentId: string, recentWindow = 50) {
  const qs = new URLSearchParams({ recent_window: String(recentWindow) }).toString();
  return apiFetch(`/v1/agents/${encodeURIComponent(agentId)}/baseline?${qs}`, token);
}
...
export async function adminDemoRequests(
  token: string,
