...
function _getInstallId(): string {
  try {
    ...
    const { mkdirSync, readFileSync, writeFileSync, existsSync } =
...
export class ZizkaDB {
  ...
  constructor(config: ZizkaDBConfig) {
    ...
    if (!apiKey && config.host) {
      if (isLocalHost(config.host)) {
        apiKey =
          apiKeyFromEnv() ?? process.env.DEV_API_KEY ?? DEFAULT_DEV_API_KEY;
      } else {
  ...
  async query(options: QueryOptions): Promise<AgentEvent[]> {
    ...
    const res = (await this.get('/v1/events', params)) as Record<
      string,
      unknown
    >[];
    ...
    return {
      ...
      print() {
        ...
        chain.forEach((event: AgentEvent, i: number) => {
          ...
          console.log(
            `${indent}${connector}${event.event}: ${data}  [${time}]`,
          );
        });
      },
    };
  ...
  async memoryDiff(sessionId: string): Promise<{
    ...
    const res = (await this.get(`/v1/memory/diff/${sessionId}`, {})) as Record<
      string,
      unknown
    >;
    return {
  ...
  private async handle(res: Response): Promise<unknown> {
    ...
    if (res.status >= 400) {
      ...
      throw new ZizkaDBError(
        `ZizkaDB error (${res.status}): ${detail}`,
        res.status,
      );
    }
    return res.json();
